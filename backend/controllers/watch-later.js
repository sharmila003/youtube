...
export const removeFromWatchLater = async (req, res) => {
  ...
  try {
    const deletedVideo = await WatchLater.findOneAndDelete({ userId: req.user.id, videoId });
    if (!deletedVideo) {
      return res.status(404).json({ message: 'Video not found in watch later list' });
    }
    res.status(200).json({ message: 'Video removed from watch later list', video: deletedVideo });
  } catch (error) {
